            ...
            foreach (var description in provider.ApiVersionDescriptions)
            {
                options.SwaggerDoc(
                  description.GroupName,
                    new OpenApiInfo()
                    {
                        Title = $"wize.resume.odata API {description.ApiVersion}",
                        Version = description.ApiVersion.ToString(),
                        Description = "OData API for Resume." + (description.IsDeprecated ? " This API Version is deprecated." : string.Empty),
